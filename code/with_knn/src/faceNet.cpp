//EDITED 2021.11.24 16:50, OVERRITED 16:50 WITH ADDED FEATURE catch each person's frame and calculate percentage.
#include "faceNet.h"
//#define K_NN 4
...
std::string FaceNetClassifier::featureMatching(int k_NN, cv::Mat &image) {
    int K_NN = k_NN;
    string winner;
	for(int i = 0; i < (m_embeddings.size()/128); i++) {
        double minDistance[K_NN];
        int winner_index[K_NN];
        for (int j = 0; j < K_NN; j++) {
            minDistance[j] = 10. * m_knownPersonThresh;
            winner_index[j] = -1;
        }
        ...
        for (int j = 0; j < m_knownFaces.size(); j++) {
            ...
            if (currDistance < minDistance[biggest]) {
                minDistance[biggest] = currDistance;  // update minimum value to minDistance set
                printf("minimum distance updated.\n"); //line for debug
                //update index of biggest value inside 'minDistance' array
                winner_index[biggest] = j;
                float maxminDistance = std::max(std::max(std::max(minDistance[0], minDistance[1]), minDistance[2]), minDistance[3]);
                for (int k = 0; k < K_NN; k++) {
                    if (minDistance[k] == maxminDistance) {
                        biggest = k;
                        break;
                    }
                }
        ...
        vector<string> winner_name;
        int freq, count = 1;
        //string winner;
        for (int j = 0; j < K_NN; j++) {
            winner_name.push_back(m_knownFaces[winner_index[j]].className.substr(0, 3));
        }
        for (int j = 0; j < K_NN; j++) {
            freq = 1;
            for (int k = 0; k < K_NN; k++) {
                if (winner_name[j] == winner_name[k]) freq += 1;
                if (freq >= count) {
                    winner = winner_name[j];
                    count = freq;
                }
            }
        }
        ...
        if (minDistance[biggest] <= m_knownPersonThresh) {
            cv::putText(image, winner, cv::Point(m_croppedFaces[i].y1 + 2, m_croppedFaces[i].x2 - 3),
                    cv::FONT_HERSHEY_DUPLEX, 0.1 + 2*fontScaler,  cv::Scalar(0,0,255,255), 1);
        }
        else if (minDistance[biggest] > m_knownPersonThresh || winner[biggest] == -1){
