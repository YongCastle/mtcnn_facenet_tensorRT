...
int main(int argc, char* argv[])
{
    ...
    int batchSize = 1;
    int nbFrames = 0;
	// create variable that stores number of frames each LJY, LYS, HSH, newperson
	int nbFramesLYS = 0;
	int nbFramesHSH = 0;
	int nbFramesLJY = 0;
	int nbFramesNewPerson = 0;
    ...
    while (true) {
		...
		// inserted to get return value
		std::string infperson;
        infperson = faceNet.featureMatching(k_NN,frame);
		...
		if (infperson == "lys") nbFramesLYS++;
		else if (infperson == "hsh") nbFramesHSH++;
		else if (infperson == "ljy") nbFramesLJY++;
		else nbFramesNewPerson++;
    ...
    std::cout << "Counted " << nbFrames << " frames in " << double(milliseconds)/1000. << " seconds!" <<
              " This equals " << fps << "fps.\n";
	std::cout << "Appearance of LYS : " << nbFramesLYS << " frames, percentage : " << double((100*nbFramesLYS)/nbFrames) << "%\n";
	std::cout << "Appearance of HSH : " << nbFramesHSH << " frames, percentage : " << double((100*nbFramesHSH)/nbFrames) << "%\n";
	std::cout << "Appearance of LJY : " << nbFramesLJY << " frames, percentage : " << double((100*nbFramesLJY)/nbFrames) << "%\n";
	std::cout << "Appearance of New Person : " << nbFramesNewPerson << " frames, percentage : " << double((100*nbFramesNewPerson)/nbFrames) << "%\n";
