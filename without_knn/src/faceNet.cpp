...
std::string FaceNetClassifier::featureMatching(cv::Mat &image) {
	int winner = 0;
    for(int i = 0; i < (m_embeddings.size()/128); i++) {
        ...
        if (minDistance <= m_knownPersonThresh) {
            cv::putText(image, m_knownFaces[winner].className.substr(0, 3), cv::Point(m_croppedFaces[i].y1+2, m_croppedFaces[i].x2-3),
	...
	return m_knownFaces[winner].className.substr(0, 3);
}
